...
const TeaListItem = memo(({ 
  ...
  teaTypes: any;
  collectionPhrases: any;
  myLogs: any;
  isOwner: boolean;
  activeTimerTeaId: number | null;
  setActiveTimerTeaId: (id: number | null) => void;
  updateLog: any;
  deleteLog: any;
  setLocation: any;
  teaGrades: TeaGrade[];
  ...
  const setTimerOpen = (open: boolean) => {
    if (open) setActiveTimerTeaId(log.tea.id);
    else if (activeTimerTeaId === log.tea.id) setActiveTimerTeaId(null);
  };
  ...
  const getPhrase = () => {
    ...
    const phrase = collectionPhrases.find(
      (p: any) => p.ownerStatus === ownerStatus && p.visitorStatus === visitorStatus
    );
...
export default function MyList() {
  ...
  const [activeTimerTeaId, setActiveTimerTeaId] = useState<number | null>(null);
  ...
  const renderLogList = (logItems: any[]) => (
    <div className="grid gap-4" style={{ gridTemplateColumns: '1fr' }}>
      {logItems.map(log => (
        <TeaListItem 
          ...
          username={username}
          teaTypes={teaTypes}
          collectionPhrases={collectionPhrases}
          myLogs={myLogs}
          isOwner={isOwner || false}
          activeTimerTeaId={activeTimerTeaId}
          setActiveTimerTeaId={setActiveTimerTeaId}
          updateLog={updateLog}
          deleteLog={deleteLog}
